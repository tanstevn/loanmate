...
export class Mediator {
    ...
    async send<TRequest extends IRequest<TResponse>, TResponse>(request: TRequest): Promise<TResponse> {
        ...
        const requestHandler = this.container.get(handlerSymbol!) as IRequestHandler<TRequest, TResponse>;
        ...
        this.behaviors.forEach((behaviorSymbol) => {
            const behavior = this.container.get(behaviorSymbol) as IPipelineBehavior<TRequest, TResponse>;
